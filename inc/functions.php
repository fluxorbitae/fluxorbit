...
*	Register action
*/
add_action( 'nextmind_action_get_breadcrumb', 'nextmind_get_breadcrumb' );
add_action( 'nextmind_action_social_sharing', 'nextmind_get_social_sharing_icons' );

function nextmind_get_social_sharing_icons() {
	$sharing_links = nextmind_generate_social_share_links();
	if($sharing_links) {
 ...
 * Add class to body
*/
add_filter( 'body_class', 'nextmind_body_class' );
function nextmind_body_class( $classes ) {
	...
	if( !get_theme_mod( 'show_small_heading_icon', $NEXTMIND_STORAGE['show_small_heading_icon'] ) ) { 
        $classes[] = 'nextmind-hide-small-icon';
    }

	if( get_theme_mod( 'custom_fancy_scrollbar', $NEXTMIND_STORAGE['custom_fancy_scrollbar'] ) ) { 
        $classes[] = 'fancy-scrollbar';
    }
	
	if($nextmind_active_demo) {
		$classes[] = $nextmind_active_demo;
	}
	
    return $classes;
}

add_action( 'wp_body_open', 'nextmind_wp_body_open' );

function nextmind_wp_body_open() {
	global $NEXTMIND_STORAGE;

	if( !is_admin() && get_theme_mod( 'show_preloader', $NEXTMIND_STORAGE['show_preloader'] ) ) { 
	
	$icon = get_theme_mod( 'preloader_icon', $NEXTMIND_STORAGE['preloader_icon'] );
	$preloader_icon = NEXTMIND_THEME_URL.'/assets/images/loader.svg';
	if ( !empty($icon) ) { 
...
function nextmind_add_additional_class_on_li($classes, $item, $args) {
	if (property_exists($args, 'li_class')) {
        $classes[] = $args->li_class;
    }
    return $classes;
}
add_filter('nav_menu_css_class', 'nextmind_add_additional_class_on_li', 1, 3);

/*
* Add class to header menu li a tag
*/

function nextmind_add_additional_class_to_a( $atts, $item, $args ) {
  if (property_exists($args, 'a_tag_class')) {
    $atts['class'] = $args->a_tag_class;
  }
  return $atts;
}
add_filter( 'nav_menu_link_attributes', 'nextmind_add_additional_class_to_a', 1, 3 );
...
function nextmind_fallback( $args ) {
	...
	$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Add a menu', 'nextmind' ) . '">' . esc_html__( 'Add a menu', 'nextmind' ) . '</a></li>';
	$fallback_output .= '</ul>';
...
if ( ! function_exists( 'nextmind_get_archive_title' ) ) {
	function nextmind_get_archive_title() {

		if ( is_front_page() ) {
			$title = esc_html__( 'Home', 'nextmind' );
		} elseif ( is_home() ) {
			$title = get_theme_mod( 'blog_page_title' );
			if(empty($title)) {
				$title = esc_html__( 'Blog', 'nextmind' );
			}
		} elseif ( is_author() ) {
			$curauth = ( get_query_var( 'author_name' ) ) ? get_user_by( 'slug', get_query_var( 'author_name' ) ) : get_userdata( get_query_var( 'author' ) );
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Author: %s', 'nextmind' ), $curauth->display_name );
		} elseif ( is_404() ) {
			$title = esc_html__( 'URL not found', 'nextmind' );
		} elseif ( is_search() ) {
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Search: %s', 'nextmind' ), get_search_query() );
		} elseif ( is_day() ) {
			// Translators: Add the queried date to the title
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'nextmind' ), get_the_date() );
		} elseif ( is_month() ) {
			// Translators: Add the queried month to the title
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'nextmind' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			// Translators: Add the queried year to the title
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'nextmind' ), get_the_date( 'Y' ) );
		} elseif ( is_category() ) {
			$title = single_cat_title( '', false );
		} elseif ( is_tag() ) {
			// Translators: Add the tag's name to the title
			$title = sprintf( esc_html__( 'Tag: %s', 'nextmind' ), single_tag_title( '', false ) );
		} elseif ( is_tax() ) {
		...
		elseif ( is_post_type_archive('awaiken-project') ) {
			...
			if(empty($title)) {
				$title = esc_html__( 'Our Projects', 'nextmind' );
			}
		}
		elseif ( is_post_type_archive() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'nextmind' ), get_the_title() );
		} elseif ( is_single() || is_page() ) {
			$title = get_the_title();
		} else {
			$title = get_the_title();
		}
		return apply_filters( 'nextmind_filter_get_archive_title', $title );
	}
}
 ...
 */
if ( ! function_exists( 'nextmind_social_icons_list' ) ) {
	function nextmind_social_icons_list() {
		...
		return apply_filters( 'nextmind_social_icons', $social_icons );
	}
}
...
if ( ! function_exists( 'nextmind_get_social_media' ) ) {
	function nextmind_get_social_media() {
		global $NEXTMIND_STORAGE;
		$output = array();
		$social_icons = nextmind_social_icons_list();
		$social_urls = explode( ',', get_theme_mod( 'social_urls', $NEXTMIND_STORAGE['social_urls'] ) );
		...
		if ( !empty( $output ) ) {
			$output = apply_filters( 'nextmind_social_profile_list', $output );
 ...
 * Social share links
 */
if ( ! function_exists( 'nextmind_generate_social_share_links' ) ) {
	function nextmind_generate_social_share_links() {
		
		global $NEXTMIND_STORAGE;
		$social_sharing_links = explode( ',', get_theme_mod( 'social_sharing', $NEXTMIND_STORAGE['social_sharing'] ) );
...
if ( ! function_exists( 'nextmind_custom_excerpt_length' ) ) {
	function nextmind_custom_excerpt_length( $length ) {
		return apply_filters( 'nextmind_excerpt_length', 16 );
	}
}

add_filter( 'excerpt_length', 'nextmind_custom_excerpt_length', 999 );

if ( ! function_exists( 'nextmind_render_icon_tag' ) ) {
    function nextmind_render_icon_tag( $file ) {
        return '<i class="fa-solid fa-arrow-right"></i>';
    }
}

if ( ! function_exists( 'nextmind_render_svg' ) ) {
    function nextmind_render_svg( $file ) {
        $valid_svg = false;
        ...
        if ( false === $valid_svg ) {
            $valid_svg = nextmind_render_icon_tag( $file );
        }
