 ...
 * @subpackage Example
 * @version    2.6.1 for parent theme Nextmind for publication on ThemeForest
...
function nextmind_json_error_license_invalid() {
    echo '<div class="notice notice-error is-dismissible">
        <p>' . esc_html( 'Invalid license', 'nextmind' ) . '</p>
    </div>';
}

function nextmind_json_error_notice() {
    echo '<div class="notice notice-error is-dismissible">
        <p>' . esc_html( 'Unable to connect to Awaiken server. Failed to retrieve valid JSON data.', 'nextmind' ) . '</p>
    </div>';
}

function nextmind_http_error_notice() {
    echo '<div class="notice notice-error is-dismissible">
		<p>' . esc_html( 'Unable to connect to Awaiken server. Failed to fetch data from the server.', 'nextmind' ) . '</p>
    </div>';
}

function nextmind_get_plugin_versions() {
    ...
    if ( !is_wp_error( $response ) && wp_remote_retrieve_response_code( $response ) === 200 ) {
        ...
        if ( $data !== null && isset( $data['data'] ) ) {
            ...
            add_action('admin_notices', 'nextmind_json_error_license_invalid');
            return null;
		} 
		else {
            // Show an admin notice if JSON structure is unexpected
            add_action('admin_notices', 'nextmind_json_error_notice');
            return null;
        }
    } else {
        // Show an admin notice if the HTTP request fails
        add_action('admin_notices', 'nextmind_http_error_notice');
        return null;
    }
}
require_once NEXTMIND_THEME_DIR . '/inc/class-tgm-plugin-activation.php';

add_action( 'tgmpa_register', 'nextmind_register_required_plugins' );
 ...
 */
function nextmind_register_required_plugins() {
	/*
	 * Array of plugin arrays. Required keys are name and slug.
	 * If the source is NOT from the .org repo, then source is also required.
	 */
	 
	$plugin_versions = nextmind_get_plugin_versions();
	...
	if ( $plugin_versions !== null ) {
		...
		$elementskit_source_url 	= get_template_directory() . '/assets/plugins/elementskit-3.8.9.zip';
		$elementskit_version 		= '3.8.9';
	}

	$plugins = array(
		array(
			'name' 				=> esc_html__('Elementor', 'nextmind'),
			'slug' 				=> 'elementor',
			'required' 			=> true,
			'external_url' 		=> 'https://wordpress.org/plugins/elementor/',
		),
		array(
			'name' 				=> esc_html__('ElementsKit Lite', 'nextmind'),
			'slug' 				=> 'elementskit-lite',
			'required' 			=> true,
			'external_url' 		=> 'https://wordpress.org/plugins/elementskit-lite/',
		),
		array(
			'name' 				=> esc_html__('ElementsKit Pro', 'nextmind'),
			'slug' 				=> 'elementskit',
		...
		array(
			'name'              => esc_html__('Nextmind Theme Addons', 'nextmind'), 
			'slug'              => 'nextmind-theme-addons',
			'source'            => get_template_directory() . '/assets/plugins/nextmind-theme-addons.zip', 
			'version'			=> '1.0.0',
			'required'          => true, 
		),
		array(
			'name'      		=> esc_html__('Contact Form 7', 'nextmind'),
			'slug'      		=> 'contact-form-7',
			'required'  		=> true,
		),
		array(
			'name'     			=> esc_html__('One Click Demo Import', 'nextmind') ,
	 ...
	 */
	$config = array(
		'id'           => 'nextmind',                 // Unique ID for hashing notices for multiple instances of TGMPA.
